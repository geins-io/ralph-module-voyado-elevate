    ...
    actions: {
      ...
      transformProducts({ commit }, products) {
        const transformedData = products.map(product => {
          ...
          try {
            products = product.products.map(prod => {
              ...
              return {
                ...
                variantDimensions: ralph_data.variantDimensions,
                firstAvailableOn: ralph_data.firstAvailableOn,
                productImages: ralph_data.productImages,
          ...
          return {
            ...product,
            products: products.length > 0 ? products : [product.products[0]]
          };
        });
  ...
  app.store.subscribe((mutation, state) => {
    if (mutation.type === 'events/push') {
      ...
      if (eventType.includes('favorite')) {
        const productKey = eventData.product?.voyadoData?.products?.[0].key;
